    ...
    >
      ...
      <div className="mb-12">
        <div className="inline-flex items-center gap-2 rounded-full border border-indigo-500/20 bg-indigo-500/10 px-4 py-1.5 text-sm font-medium text-indigo-600 dark:text-indigo-300">
          ...
          <span className="uppercase tracking-wider">Developer Guide</span>
        </div>

        <h2 id="setup-heading" className="text-3xl sm:text-4xl font-bold text-slate-900 dark:text-white mt-6 mb-4">
          Developer Setup
        </h2>
        <p className="text-slate-600 dark:text-gray-400 max-w-2xl leading-relaxed">
      ...
      >
        <div className="flex flex-col sm:flex-row items-start gap-5">
          ...
          <div className="flex-1">
            <h3 className="text-lg font-bold text-slate-900 dark:text-white">Prerequisites</h3>
            <p className="mt-2 text-sm leading-relaxed text-slate-600 dark:text-slate-400">
              Before starting, ensure you have{" "}
              <span className="font-semibold text-slate-800 dark:text-slate-200">Node.js 18+</span>,{" "}
              <span className="font-semibold text-slate-800 dark:text-slate-200">npm 9+</span>, and a running MongoDB instance configured locally or in the cloud.
            </p>

            <div className="mt-4 flex flex-wrap gap-3">
              <span className="rounded-full border border-indigo-500/20 bg-indigo-500/10 px-3 py-1 text-xs font-medium text-indigo-600 dark:text-indigo-300">Node.js 18+</span>
              <span className="rounded-full border border-blue-500/20 bg-blue-500/10 px-3 py-1 text-xs font-medium text-blue-600 dark:text-blue-300">npm 9+</span>
              <span className="rounded-full border border-purple-500/20 bg-purple-500/10 px-3 py-1 text-xs font-medium text-purple-600 dark:text-purple-300">MongoDB</span>
            </div>
            ...
            <div className="mt-5 rounded-2xl border border-amber-500/20 bg-amber-500/10 p-4">
              <div className="flex items-start gap-3">
                ...
                <p className="text-sm leading-relaxed text-amber-700 dark:text-amber-300">
                  Always copy <code className="rounded bg-black/10 px-1.5 py-0.5 font-mono text-xs dark:bg-white/10">.env.example</code> to <code className="rounded bg-black/10 px-1.5 py-0.5 font-mono text-xs dark:bg-white/10">.env</code> and never commit real environment variables or API keys.
                </p>
              </div>
            </div>
          </div>
