      ...
      <div className="relative rounded-2xl border border-white/10 bg-gradient-to-br from-[#0f172a]/90 to-[#111827]/90 backdrop-blur-md shadow-2xl shadow-blue-500/10 p-8 overflow-hidden">
        ...
        <div className="relative z-10">
          <div className="mb-8">
            ...
            <p className="text-sm text-gray-400 mt-1">Your feedback helps us improve StorySparkAI for everyone.</p>
          </div>

          {success && (
            <div aria-live="polite" className="mb-6 flex items-center gap-3 p-4 bg-green-500/10 border border-green-500/20 text-green-400 rounded-xl text-sm animate-fade-in">
          ...
          {errors.submit && (
            <div aria-live="polite" className="mb-6 flex items-center gap-3 p-4 bg-red-500/10 border border-red-500/20 text-red-400 rounded-xl text-sm">
          ...
          <div className="space-y-5">
            ...
            <div>
              <label htmlFor="role" className="flex items-center gap-2 text-sm font-medium text-gray-300 mb-2">
                <span className="text-blue-400">💼</span>
              ...
              <input
                ...
                className="w-full px-4 py-3 rounded-xl border border-white/10 bg-white/5 text-white placeholder-gray-500 focus:outline-none focus:border-blue-500/60 focus:ring-2 focus:ring-blue-500/20 transition-all duration-200"
              />
              {errors.role && <p className="text-red-400 text-xs mt-1.5 flex items-center gap-1"><span>⚠</span> {errors.role}</p>}
            </div>

            <div>
              <label htmlFor="feedback" className="flex items-center gap-2 text-sm font-medium text-gray-300 mb-2">
                <span className="text-blue-400">💬</span>
              ...
              <div className="flex justify-between items-center mt-1">
                {errors.feedback ? <p className="text-red-400 text-xs flex items-center gap-1"><span>⚠</span> {errors.feedback}</p> : <span />}
                <p className={`text-xs ${feedback.length > 450 ? "text-yellow-400" : "text-gray-500"}`}>{feedback.length}/500</p>
              </div>
            </div>
            ...
            <div>
              ...
              {errors.rating && <p className="text-red-400 text-xs mt-1.5 flex items-center gap-1"><span>⚠</span> {errors.rating}</p>}
            </div>
