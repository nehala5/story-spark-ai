  ...
  try {
    const response = await executeWithRetryAndFallback(async (activeModel) => {
      ...
      return chatSession.sendMessage(
        `${buildGenreInstruction(genre)}${TONE_INSTRUCTIONS[tone || "Dramatic"]}${buildCharactersInstruction(characters)}You are an expert storyteller and emotion analyst. The user provided the following base prompt: "${prompt}".
